...
//
class Oven {
  constructor(cookie, endBake){
    this.cookie = cookie
    this.startBake = 0
    this.endBake = endBake
    this.status = ''

  }

  baking(countDowntTimer){

    while(this.startBake < countDowntTimer){
      this.startBake++

      if(this.startBake >= this.endBake){
        this.status = "hangus"
      }
      else if(this.startBake === this.endBake-5){
        this.status = "matang"
      }else if(this.startBake === this.endBake - 10){
...
let oven = new Oven (batch_of_cookies[2], 25 )

console.log(oven.baking(20));
